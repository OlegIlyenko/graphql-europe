...
class Conference2017 @Inject() (config: Configuration) extends ConferenceData {
  val baseUrl = config.get[String]("graphqlEurope.canonicalUrl")

  val conf = config.underlying.as[Config]("graphqlEurope")

  def cleanupText(s: String) = s.stripMargin.replaceAll("\r\n", "\n")

  def url(path: String) = baseUrl + "/2017" + path
  def assetUrl(path: String) = baseUrl + "/assets/2017/image" + path
  ...
  val conference = Conference(
    ...
    description = "Join Europe’s first GraphQL conference",
    shortDescription = "Join Europe’s first GraphQL conference",
    location = "Berlin, May 21st, 2017",
    ...
    team = team,
    tickets = tickets,
    sponsorEmail = config.get[String]("graphqlEurope.sponsorEmail"),
    supportEmail = config.get[String]("graphqlEurope.supportEmail"),
    ticketsUrl = "https://graphql-europe-2017.eventbrite.com#tickets",
    url = url(""),
